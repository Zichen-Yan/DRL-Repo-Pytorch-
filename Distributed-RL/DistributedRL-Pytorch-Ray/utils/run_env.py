...
def run_env(env, brain, traj_length=0, get_traj=False, reward_scaling=0.1, test=False):
    score = 0
    transition = None
    if traj_length == 0:
...
def test_agent(env_name, agent, ps, repeat, sleep=3):
    ...
    while True:
        ...
        for i in range(repeat):
            score = run_env(env, agent, test=True)
            score_list.append(score)
        print("time : ", total_time, "'s, ", repeat, " means performance : ", sum(score_list) / repeat)
        if sleep == 0:
            return
