    ...
    def stack_data(self):
        ...
        for i in range(size):
            priority, state, action, reward, next_state, done = \
                data[i]['priority'], data[i]['state'], data[i]['action'], data[i]['reward'], data[i]['next_state'], \
                data[i]['done']
...
class ImpalaBuffer:
    ...
    def sample(self):
        ...
        state, action, reward, next_state, done, log_prob = np.stack(state), np.stack(action), np.stack(
            reward), np.stack(next_state), np.stack(done), np.stack(log_prob)
