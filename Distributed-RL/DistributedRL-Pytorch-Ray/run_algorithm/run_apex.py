...
def run(args, agent_args):
    ...
    ray.get([agent.init.remote(idx, algorithm(writer, device, state_dim, action_dim, agent_args, epsilon=(
                agent_args['epsilon'] ** (1 + (idx / (args.num_actors - 1)) * agent_args['alpha']))), agent_args) for
             idx, agent in enumerate(actors)])
    ...
    test_agent.remote(args.env_name, algorithm(writer, device, state_dim, action_dim, agent_args, epsilon=0), ps,
                      args.test_repeat, args.test_sleep)
