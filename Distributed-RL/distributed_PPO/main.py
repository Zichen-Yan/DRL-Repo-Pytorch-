...
import mujoco_py
import os
from utils import TrafficLight, Counter, Shared_grad_buffers, Running_mean_filter, RewardCounter
...
def train(args):
    ...
    p = mp.Process(target=chief, args=(args, traffic_light, counter, num_of_workers,
                                       shared_model, shared_grad_buffers, optimizer, shared_obs_filter,shared_reward))
    processes.append(p)

    for idx in range(num_of_workers):
        workers.append(PPO_worker(idx, args, traffic_light, counter, shared_model,
                                  shared_obs_filter, shared_grad_buffers, shared_reward))
