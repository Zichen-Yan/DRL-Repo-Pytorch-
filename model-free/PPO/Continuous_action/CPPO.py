...
def train(args):
    ...
    agent = PPO(num_state=num_state, num_action=num_action, use_GAE=args.use_GAE, action_bound=action_bound)
    if args.eval:
        agent.load_params()
        print('Load Success!')
    sum_reward = []
    ...
    for episode in range(args.episodes):
        ...
        for t in range(201):
            if args.render:
                env.render()
            counter += 1
            action, action_prob = agent.action(state)
