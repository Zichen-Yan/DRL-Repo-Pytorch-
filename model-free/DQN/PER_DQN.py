...
class DQN(object):
    ...
    def __init__(self, num_state, num_action, batch_size=32, lr=1e-3, memory_size=2000, Transition=None):
        ...
        self.memory = Memory(self.memory_size)  # s,s_,reward,action
    ...
    def learn(self):
        if self.memory_counter>=self.memory_size:
            ...
            is_weight = torch.FloatTensor(is_weight).view(-1, 1)

            # q_eval w.r.t the action in experience
            q_eval = self.eval_net(b_s).gather(1, b_a)
            q_next = self.target_net(b_s_).detach()
            q_target = b_r + self.GAMMA * q_next.max(1)[0].view(self.batch_size, 1)*(1-b_d)
            elementwise_loss = F.smooth_l1_loss(q_eval, q_target, reduction="none")
            loss = torch.mean(elementwise_loss * is_weight)
