...
class ActorNet(nn.Module):
    def __init__(self, num_action, num_state, hidden_layer=64, a_bound=1):
        ...
        self.fc2 = nn.Linear(hidden_layer, num_action)
...
class DDPG(object):
    def __init__(self, n_a, n_s, a_bound, memory_size=100000, batch_size=64):
        self.a_dim, self.s_dim, self.a_bound = n_a, n_s, a_bound
        self.memory = np.zeros((memory_size, n_s * 2 + n_a + 1), dtype=np.float32)
        self.pointer = 0  # serves as updating the memory data
        # Create the 4 network objects
        self.actor_eval = ActorNet(n_a, n_s)
        self.actor_target = ActorNet(n_a, n_s)
        self.critic_eval = CriticNet(n_a, n_s)
        self.critic_target = CriticNet(n_a, n_s)
    ...
    def load_params(self):
        self.actor_eval.load_state_dict(torch.load('./result/actor_eval_net_params.pkl'))
        self.actor_target.load_state_dict(torch.load('./result/actor_target_net_params.pkl'))
        self.critic_eval.load_state_dict(torch.load('./result/critic_eval_net_params.pkl'))
        self.critic_target.load_state_dict(torch.load('./result/critic_target_net_params.pkl'))

    def save_param(self):
        torch.save(self.actor_eval.state_dict(), './result/actor_eval_net_params.pkl')
        torch.save(self.actor_target.state_dict(), './result/actor_target_net_params.pkl')
        torch.save(self.critic_eval.state_dict(), './result/critic_eval_net_params.pkl')
        torch.save(self.critic_target.state_dict(), './result/critic_target_net_params.pkl')
