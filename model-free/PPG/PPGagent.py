...
class PPG:
    def __init__(
        ...
        self.actor = Actor(state_dim, actor_hidden_dim, num_actions).to(self.device)
        self.critic = Critic(state_dim, critic_hidden_dim).to(self.device)
    ...
    def learn(self, memories, aux_memories, next_state):
        ...
        for mem in memories:
            ...
            old_log_probs.append(torch.tensor(mem.action_log_prob))
            rewards.append(torch.tensor(mem.reward))
            masks.append(1 - float(mem.done))
            values.append(torch.tensor(mem.value))
