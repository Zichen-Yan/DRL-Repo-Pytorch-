...
def train(args, env_sampler, predict_env, agent, env_pool, model_pool):
    total_step = 0
    rollout_length = 1
    exploration_before_start(args, env_sampler, env_pool, agent)  # 预存5000条真实数据
    reward_list = []
    for epoch_step in range(args.num_epoch):
        ...
        for i in count():
            ...
            if cur_step > 0 and cur_step % args.model_train_freq == 0 and args.real_ratio < 1.0 and not args.eval:
                first_time = time.time()
                train_predict_model(args, env_pool, predict_env)
                time_ = time.time()-first_time
                print('time_train_env_model:', time_)
            ...
            cur_state, action, next_state, reward, done, info = env_sampler.sample(agent, eval_t=args.eval, render=args.render)
            env_pool.push(cur_state, action, reward, next_state, done)

            # train policy
            if len(env_pool) > args.min_pool_size and not args.eval:
                train_policy_steps += train_policy_repeats(args, total_step, train_policy_steps, cur_step, env_pool, model_pool, agent)

            total_step += 1

            # Evaluation
            if total_step % 2000 == 0 and not args.eval:
                ...
                # if sum_reward >= 1500:
                #     agent.save_model(env_name=args.env_name)
...
def rollout_model(args, predict_env, agent, model_pool, env_pool, rollout_length):
    state, action, reward, next_state, done = env_pool.sample_all_batch(args.rollout_batch_size)  # 没有足够样本也会复制填满
...
def train_policy_repeats(args, total_step, train_step, cur_step, env_pool, model_pool, agent):
    ...
    for i in range(args.num_train_repeat):  # 每个step训练20遍 batch_size=256
...
def main(args=None):
    ...
    # Initial pool for env 两个记忆库
    env_pool = ReplayMemory(args.replay_size)  # 1000000
    # Initial pool for model
    rollouts_per_epoch = args.rollout_batch_size * args.epoch_length / args.model_train_freq
    model_steps_per_epoch = int(1 * rollouts_per_epoch)
    new_pool_size = args.model_retain_epochs * model_steps_per_epoch
    model_pool = ReplayMemory(new_pool_size)  # 100000*1000/250*1=400000
